...
# Deploy RGW first, then Ingress (register for cleanup on failure)
deploy_service "${SPEC_DIR}/rgw.yaml" "rgw.rgw_only_pt"
register_cleanup service "rgw.rgw_only_pt"
wait_for_service "rgw.rgw_only_pt"

deploy_service "${SPEC_DIR}/ingress.yaml" "ingress.ingress.passthrough"
register_cleanup service "ingress.ingress.passthrough"
